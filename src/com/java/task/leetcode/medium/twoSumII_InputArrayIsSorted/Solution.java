package com.java.task.leetcode.medium.twoSumII_InputArrayIsSorted;
...
public class Solution {
    ...
    public static int[] twoSum2(int[] numbers, int target) {
        ...
        while (start < end){
            int match = numbers[start] + numbers[end];
            if(match == target) return new int[]{start + 1, end + 1};
            else if (match > target) end--;
            else start++;
        }
        return new int[0];
    }
